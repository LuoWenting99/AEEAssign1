function [TckResultCT_POS, navSolutionsCT] = trackingCT_POS(file,signal,track,cmn, Acquired,TckResult_Eph,cnslxyz,eph,sbf,solu)
%Purpose:
...
%
%Outputs:
%	TckResultCT_POS         - conventional tracking results
...
if solu.mode == 1
    num_state_ekf = 8;    
    state_ekf = [cnslxyz(1:3), zeros(1,3), 0, 0]';  % [position,velocity,clock bias,clock drift ]
    cov_ekf = diag([1e4, 1e4, 1e4, 1e1, 1e1, 1e1, 1e4, 1e2]); 
    
    dt = track.pdi * signal.ms;  % unit, seconds
    % process noise Q
    Sv = 1e-21;  % dynamic noise
    Q_pos = Sv * (dt^3/3) * eye(3);
    Q_vel = Sv * dt * eye(3);
    Q_clk_bias = 1e-3 * dt;
    Q_clk_drift = 1e-4 * dt;
    Q_ekf = blkdiag(Q_pos, Q_vel, Q_clk_bias, Q_clk_drift);
    
    % measuement noise initialization
    R_ekf = diag([ones(1, svlength)*3e4, ones(1, svlength)*1e0]);
...
%                                                 + (1/cmn.cSpeed)*sv_clk(prn);
            ...
            prvec(svindex)   = pseudorange(svindex) - sv_clk(prn) - grpdel*cmn.cSpeed;% -sv_clk(prn)?£º
        ...
        if solu.mode==0
            %WLS positioning
            cn0 = Acquired.SNR
            T_coh = track.pdi * signal.ms;  % Coherent integration time (seconds)
            sigma = 3e8 ./ sqrt(2 * T_coh * 10.^(cn0/10));  % calculate standard deviation
            sigma = max(sigma, 1e-6);
            weight = 1 ./ (sigma.^2);  % weight vector
            [estusr_wls, dop] = wlspos(prvec, svxyzr, estusr_wls, 1e-3, weight);     
            [VR, dtRV, ~]     = ...
            LS_SA_code_Vel_xubing(estusr_wls(1:3)', svxyzr, sv_vel, carrFreq', 0.190293672798365, sv_clk_vel(sv));
            VR = VR'; % Convert to column vector 
            usr_clk_wls = estusr_wls(4); %set clock bias
      
        elseif solu.mode==1         
            % EKF positioning   
            doppler_mps = (carrFreq - signal.IF) * cmn.cSpeed / signal.Fc- sv_clk_vel(svindex);  % Units£ºm/s
            
            cn0 = CN0_CT(posIndex, :);  
            T_coh = track.pdi * signal.ms;         
            sigma_pr = 3e8 ./ sqrt(2 * T_coh * 10.^(cn0/10));  % calculate standard deviation
            sigma_doppler = 0.05;   % Doppler noise standard deviation£¨Hz£©        
            R_pr = diag(sigma_pr.^2);
            R_doppler = diag(power(sigma_doppler,2) * ones(1, svlength));
            R_ekf = blkdiag(R_pr, R_doppler);  
            
            % if parameter are not ready, use default initial position
            state_ekf = [cnslxyz(1:3), zeros(1,5)]';
            warning('WLS initialization parameters not found, using default initial state');
                     
            % EKF updated parameter
            dt = pdi * signal.ms; 
            % Use EKF update
            [state_ekf, cov_ekf] = ekf_update(...
                prvec, doppler_mps, svxyzr, sv_vel, sv_clk_vel(sv), ...
                state_ekf, cov_ekf, Q_ekf, R_ekf, dt);

            % Save EKF result to variable  
            estusr_wls = state_ekf(1:3)';    % position
            VR = state_ekf(4:6)';            % velocity
            usr_clk_wls = state_ekf(7);      % clock bias
            dtRV = state_ekf(8);             % clock drift
        end
...
%         usr_velENU(posIndex,:) = C_e_n * VR ;
        usr_velENU(posIndex,:) = C_e_n * VR';  %convert VR to column vector
...
if solu.mode == 0
    ...
    save(['tckRstCT_WLS',num2str(pdi),'ms_',file.fileName], 'TckResultCT_POS','CN0_CT');
    fprintf('WLS result saves to : %s\n', wls_filename);
elseif solu.mode == 1
    % save EKF result
    ekf_fg2str(pdi),'ms_',file.fileName], 'TckResultCT_POS','CN0_CT');
    fprintf('EKF result seve t0: %s\n', ekf_filename);
end
